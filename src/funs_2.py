...
class Pipeline:
    ...
    def generate_thinking(self, data: List[Dict]) -> List[Dict]:
        """
        フェーズ2: 思考生成
        """
        self._require_init()
        ...
        if settings.Using_think_models_for_thinking:
            model = self.inf.think_model_load(settings)
            inference_func = self.inf.think_model_inference
            model_name_for_log = settings.think_model_name
        else:
        ...
        pbar = tqdm(total=len(data), desc="思考生成中")
        # ここに思考生成promptを読み込み
        thinking_prompt = self.prompts["thinking_prompt"]
        # print(data)
        ...
        for batch in batched(data, settings.batch_size):
            ...
            for i, res in enumerate(results):
                ...
                if settings.Using_think_models_for_answer:
                    ...
                    # d["think"] = think_part
                    # d["Answer"] = answer_part
                    d["thinking"] = answer_part
                else:
                    d["thinking"] = res
                thinking_data.append(d)
            pbar.update(len(batch))
        pbar.close()
    ...
    def evolve_thinking(self, data: List[Dict]) -> List[Dict]:
        ...
        for stage_idx in range(1, times + 1):
            ...
            for batch in batched(data_to_evolve, settings.batch_size):
                batch_prompts = [f"{evo_prompt.format(academic=d['Academic'], Question=d['Question'], thinking=d['thinking'], answer=d['Answer'])}" for d in batch]
    ...
    def evolve_answers(self, data: List[Dict]) -> List[Dict]:
        ...
        ################################################################### 改造箇所
        # instruction = "Please answer the following question in English."
        answering_prompt = self.prompts["evo_answer"]
        # print(f"回答進化プロンプト: {answering_prompt}")
        ###################################################################
        ...
        for stage_idx in range(1, times + 1):
            ...
            for batch in batched(data_to_evolve, settings.batch_size):
                batch_prompts = [f"{answering_prompt.format(academic=d['Academic'], Question=d['Question'], thinking=d['thinking'], answer=d['Answer'])}" for d in batch]
                # batch_prompts = [f"{evo_prompt}\n\n{d[data_key]}" for d in batch]
                evolved_texts = self.inf.inst_model_inference(model, batch_prompts, settings)
                for i, d in enumerate(batch):
                    new_d = d.copy()
    ...
    def save_dataset(self, data: List[Dict], path: Optional[Union[str, Path]] = None) -> Path:
        ...
        for item in data:
            formatted_item = {
                ...
                "generator": f"Q:{settings.Instruct_model_name}/T:{settings.think_model_name}/A:{settings.think_model_name}",
